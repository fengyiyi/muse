...
(function() {
  ...
  make_cards = function(box) {
    ...
    card = box.selectAll('div.card').data(function(d) {
      if (d) {
        return cross({
          card: drawerState[d].cards.map(function(c) {
            return c.text;
          }),
          pos: [drawerPos[d]]
        });
  ...
  (function() {
    ...
    makeWriter = function(drawer, delay) {
      ...
      setTimeout((function() {
        return makeCard('', function() {
          ...
          }).style('width', drawerWidth + 'px').style('height', drawerHeight + 'px').style('border-radius', '0px').transition().duration(1000).delay(500).style('left', ($totalOuterWidth - writerWidth) / 2 + 'px').style('top', ($totalOuterHeight - writerHeight) / 2 + 'px').style('width', writerWidth + 'px').style('height', writerHeight + 'px').style('border-radius', '3px').each('end', function() {
            return editor.node().focus();
          });
        });
      }), delay);
    };
    ...
    editor = writer.append('textarea').on('keyup', function() {
      updateCard(editor.property('value'), function(res) {
  ...
  (function() {
    ...
    if (window.io) {
      ...
      (function() {
        var callback;
        callback = null;
        socket.on('updateCardResult', function(res) {
          if (typeof callback === "function") {
            callback(res);
          }
          callback = null;
        });
        return updateCard = function(text, cb) {
          callback = cb;
